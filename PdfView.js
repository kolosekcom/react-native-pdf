...
export default class PdfView extends Component {
    ...
    switch (fitPolicy) {
      ...
      default: {
        if (
          ...
          return (
            this.state.contentContainerSize.height * this.state.pageAspectRate * this.state.scale
          )
        }
      }
    }
  ...
  _getPageHeight = () => {
    ...
    switch (fitPolicy) {
      case 0: //fit width
        return (
          this.state.contentContainerSize.width * (1 / this.state.pageAspectRate) * this.state.scale
        )
      case 1: //fit height
  ...
  _onScaleChanged = (pinchInfo) => {
    ...
    let newContentOffset = {
      x:
        (this.state.contentOffset.x + pinchInfo.pageX) * (newScale / this.state.scale) -
        pinchInfo.pageX,
      y:
        (this.state.contentOffset.y + pinchInfo.pageY) * (newScale / this.state.scale) -
        pinchInfo.pageY,
    }
  ...
  _renderItem = ({ item, index }) => {
    ...
    if (this.props.singlePage) {
      return (
        <View style={{ flexDirection: this.props.horizontal ? "row" : "column" }}>{pageView}</View>
      )
    }
  ...
  _onViewableItemsChanged = (viewableInfo) => {
    for (let i = 0; i < viewableInfo.viewableItems.length; i++) {
      ...
      if (
        viewableInfo.viewableItems.length + viewableInfo.viewableItems[0].index <
  ...
  _renderList = () => {
    ...
    return (
      <PdfViewFlatList
        ...
        scrollEnabled={this.props.scrollEnabled && !this.props.singlePage}
      />
    )
  }
